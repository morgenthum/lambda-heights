...
renderHudVelocity renderer config = do
  ...
  let WV (V2 x y) = Player.velocity . State.player $ state
...
renderHudScore renderer config = do
  state <- Loop.askRenderState
  let textFont = font config
  let Score score = Player.score . State.player $ state
...
shapeSize :: Shape -> WorldSize
shapeSize (Shape xs ys) = WS $ V2 (maximum xs) (maximum ys)
...
renderPlayerShadow renderer config windowSize screen player = do
  ...
  let WV (V2 velX _) = Player.velocity player
  let offX = if velX >= 0 then -20 else 20
  let shape = Shape [0, 10, 25, 10, 0, 15] [80, 80, 40, 0, 0, 40]
  let translatedShape =
        centerBottom (WP $ V2 (posX + offX) posY) $ flipByVel (Player.velocity player) shape
...
flipByVel :: WorldVel -> Shape -> Shape
...
centerBottom (WP (V2 posX posY)) (Shape xs ys) =
  let WS (V2 w _) = shapeSize (Shape xs ys)
...
layerColor layer =
  let WS (V2 w h) = Layer.size layer
      go
