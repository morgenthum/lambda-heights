...
renderPlayer renderer config windowSize screen player = do
  let shape = translateCenterBottom (Player.position player) $ flipShapeByVelocity (Player.velocity player) playerShape
...
renderPlayerShadow renderer config windowSize screen player = do
  ...
  let shape =
        translateCenterBottom (posX + offX, posY) $ flipShapeByVelocity (Player.velocity player) playerShadowShape
...
translateCenterBottom (posX, posY) (Shape.Shape xs ys) = Shape.Shape xs' ys'
 where
  (w, _) = Shape.size (Shape.Shape xs ys)
