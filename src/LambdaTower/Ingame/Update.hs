...
scrollView delta view = view {
    S.top = S.top view + delta * 1.5,
    S.bottom = S.bottom view + delta * 1.5
  }

scrollViewOverTime :: S.View -> S.View
scrollViewOverTime view = if S.bottom view == 0 then view else scrollView (deltaTime*100) view
...
updateAcceleration acceleration velocity motion =
  if S.air motion
  then updateAirAcceleration acceleration velocity motion
  else updateGroundAcceleration acceleration velocity motion

updateGroundAcceleration :: P.Acceleration -> P.Velocity -> S.Motion -> P.Acceleration
updateGroundAcceleration (V.V2 x y) (V.V2 velX velY) motion
  | jump && (velX > 750 || velX < -750 || velY > 750) = V.V2 x 250000
  | jump = V.V2 x 125000
...
updateAirAcceleration :: P.Acceleration -> P.Velocity -> S.Motion -> P.Acceleration
updateAirAcceleration (V.V2 x y) (V.V2 velX velY) motion
  | left && right = V.V2 0 (-2000)
  | left = V.V2 (-1500) (-2000)
